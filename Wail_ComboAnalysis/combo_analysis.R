...
setwd("~/Desktop/Projects/DP_Fluva_Combo/FluvaDP_paper/FluvaDP_repo/FluvaDP_repo/")
...
pdf("Plots/Fig4_A.pdf",width = 15,height = 4)
Heatmap(BlissMat_summarized_all[,order],cluster_columns = T,cluster_rows = F,top_annotation = column_ha
...
data <- as.data.frame(t(BlissMat_summarized_all[,order]),stringsAsFactors=F)

write.table(data,"../../synergy_scores.csv",sep = ",",row.names = T,col.names = NA)

data$subtype <- subtypes[rownames(data),"SCMOD2"]
data$AAC <- Fluva[rownames(data)]
#ggplot(data,aes(x=FLUVA_DP,y=FLUVA_NFV,color=AAC)) +
...
EMT_genes <- c("VIM", "CDH1", "CDH2", "ZEB1", "FN1")
EMT_genes_ENS <- rownames(gene_mappings)[match(EMT_genes,gene_mappings$Symbol)]
names(EMT_genes_ENS) <- EMT_genes

col_fun = colorRamp2(c(-.8, 0, 0.8), c("green", "gray", "yellow"))

pdf("Plots/FigS8_A_B.pdf",height = 4,width = 15)


par(mfrow=c(1,4),mgp=c(3,1.5,0))
for (i in 1:3) {
  tmp <- sort(BlissMat_summarized[i,])
  emt_exp_mat <- RNAseq[names(tmp),EMT_genes_ENS]
  emt_exp_mat <- (apply(emt_exp_mat, 2, scale))
  
  colnames(emt_exp_mat) <- EMT_genes
  rownames(emt_exp_mat) <- names(tmp)
  #  ha = HeatmapAnnotation("Bliss"=tmp,col = list(Bliss = col_fun))
  #  print(Heatmap(t(emt_exp_mat),cluster_columns = F,top_annotation = ha))
  
  binary_emt_exp_mat <- ifelse(emt_exp_mat>0,1,0)
  binary_emt_exp_mat_final <- apply(binary_emt_exp_mat, 1, function(x){
    return(any(x[c(1,3,4,5)]) | !x[2] )
  })
  rule = "Rule: RY_2018_EMT_signature"
  
  A <- wilcox.test(x=tmp[names(binary_emt_exp_mat_final)[binary_emt_exp_mat_final==T]],
                   y=tmp[names(binary_emt_exp_mat_final)[binary_emt_exp_mat_final==F]])
  test_data <- list("Rule-positive"=tmp[names(binary_emt_exp_mat_final)[binary_emt_exp_mat_final==T]]
                    ,"Rule-negative"=tmp[names(binary_emt_exp_mat_final)[binary_emt_exp_mat_final==F]])
  names(test_data) <- c(paste("Rule-positive","\nn:",length(test_data[[1]]),sep = "")
                        ,paste("Rule-negative","\nn:",length(test_data[[2]]),sep = "")) 
  boxplot(test_data
          ,col=c("white","white")
          ,main = paste(rownames(BlissMat_summarized)[i],rule,paste("P:",sprintf("%.2E",A$p.value)),sep = "\n")
          ,ylab = "Bliss Index", outpch = NA)
  stripchart(test_data, vertical = TRUE, 
             method = "jitter", add = TRUE, pch = 20, col = 'blue',cex = 1.5)
  #  legend("topleft",legend = paste("pval:",sprintf("%.2E",A$p.value)),bty = "n")
}

#tmp <- 1-Fluva
tmp <- FluvaMonocommonAvg_IC50
#  tmp[is.na(tmp) | tmp > 20] <- 20
rule = "Rule: RY_2018_EMT_signature"
CDH1 <- binary_emt_exp_mat_final
A <- wilcox.test(x=tmp[intersect(names(CDH1)[CDH1==T],names(tmp))],
                 y=tmp[intersect(names(CDH1)[CDH1==F],names(tmp))])
test_data <- list("Rule-positive"=tmp[intersect(names(CDH1)[CDH1==T],names(tmp))]
                  ,"Rule-negative"=tmp[intersect(names(CDH1)[CDH1==F],names(tmp))])
names(test_data) <- c(paste("Rule-positive","\nn:",length(test_data[[1]]),sep = "")
                      ,paste("Rule-negative","\nn:",length(test_data[[2]]),sep = "")) 
boxplot(test_data
        ,col=c("white","white")
        ,main = paste("Fluva",rule,paste("P:",sprintf("%.2E",A$p.value)),sep = "\n"),ylab="IC50"
        , outpch = NA)
...
# pdf("CDH1_biomarker.pdf",height = 6,width = 19)
# par(mfrow=c(1,3))
# for (i in 1:3) {
# tmp <- sort(BlissMat_summarized[i,])
# CDH1 <- binary_emt_exp_mat[,"CDH1"]
# boxplot(list("Sensitive (predicted)"=tmp[names(CDH1)[CDH1==F]]
#              ,"Resistant (predicted)"=tmp[names(CDH1)[CDH1==T]])
#         ,col=c("skyblue","red"),main = rownames(BlissMat_summarized)[i],ylab="Bliss Index")
# A <- wilcox.test(x=tmp[names(CDH1)[CDH1==F]],
#                  y=tmp[names(CDH1)[CDH1==T]])
# 
# legend("topleft",legend = c(paste("pval:",sprintf("%.2E",A$p.value)),"Low CDH1","High CDH1"),bty = "n"
#        ,fill = c(NA,"skyblue","red"))
# }
# 
# dev.off()
...
gene <- "CDH1"
pdf("Plots/Fig4C.pdf",height = 4,width = 15)
par(mfrow=c(1,4),mgp=c(3,1.5,0))
for (i in 1:3) {
  tmp <- sort(BlissMat_summarized[i,])
  CDH1 <- binary_emt_exp_mat[,gene]
  A <- wilcox.test(x=tmp[names(CDH1)[CDH1==F]],
                   y=tmp[names(CDH1)[CDH1==T]])
  
  test_data <- list(tmp[names(CDH1)[CDH1==F]],tmp[names(CDH1)[CDH1==T]])
  names(test_data) <- c(paste("Low ",gene,"\nn:",length(test_data[[1]]),sep = "")
                        ,paste("High ",gene,"\nn:",length(test_data[[2]]),sep = "")) 
  boxplot(test_data
          ,col=c("white","white")
          ,main = paste(rownames(BlissMat_summarized)[i],paste("P:",sprintf("%.2E",A$p.value)),sep = "\n")
          ,ylab="Bliss Index", outpch = NA)
...
CDH1 <- binary_emt_exp_mat[,gene]
A <- wilcox.test(x=tmp[intersect(names(CDH1)[CDH1==F],names(tmp))],
                 y=tmp[intersect(names(CDH1)[CDH1==T],names(tmp))])
...
boxplot(test_data
        ,col=c("white","white")
        ,main = paste("Fluva",paste("P:",sprintf("%.2E",A$p.value)),sep = "\n")
        ,ylab="IC50", outpch = NA)
stripchart(test_data, vertical = TRUE, 
           method = "jitter", add = TRUE, pch = 20, col = 'blue',cex = 1.5)
...
pdf("Plots/FigS8_C_D.pdf",height = 4,width = 15)

for (gene in EMT_genes[-2]) {
  ...
  for (i in 1:3) {
    tmp <- sort(BlissMat_summarized[i,])
    CDH1 <- binary_emt_exp_mat[,gene]
    A <- wilcox.test(x=tmp[names(CDH1)[CDH1==T]],
                     y=tmp[names(CDH1)[CDH1==F]])
    
    test_data <- list(tmp[names(CDH1)[CDH1==T]],tmp[names(CDH1)[CDH1==F]])
    names(test_data) <- c(paste("High ",gene,"\nn:",length(test_data[[1]]),sep = "")
                          ,paste("Low ",gene,"\nn:",length(test_data[[2]]),sep = "")) 
    boxplot(test_data
            ,col=c("white","white")
            ,main = paste(rownames(BlissMat_summarized)[i],paste("P:",sprintf("%.2E",A$p.value)),sep = "\n")
